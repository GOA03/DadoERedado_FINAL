...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function proximoNivel() {
        ...
        if (jogadas < 2) {
            ...
            if (operacao === 'soma') {
                perguntaElement.innerText = `QUAL É A SOMA?`;
                    perguntaElement.style.color = 'blue';
            } else {  // subtração
                perguntaElement.innerText = `QUAL É A SUBTRAÇÃO?`;
    ...
    document.querySelectorAll('#menuDificuldade button').forEach(button => {
        let audioPlaying = false;  // Variável para controlar se o áudio está atualmente sendo reproduzido
        ...
        // Adicionando evento de mouseover para tocar o áudio ao passar o cursor pela dificuldade
        button.addEventListener('mouseover', function() {
            // Verificar se o áudio já está sendo reproduzido
            if (!audioPlaying) {
                const dificuldade = this.getAttribute('data-dificuldade');
                const sound = new Audio(`sounds/${dificuldade}.mp3`);
                
                // Definir o sinalizador para true para indicar que o áudio está sendo reproduzido
                audioPlaying = true;

                // Adicionar um ouvinte de evento para redefinir o sinalizador quando o áudio terminar
    ...
    // Adicionando ouvinte de evento ao botão de ajuda
document.getElementById('toggleSound1').addEventListener('click', function() {
    const audioPergunta = new Audio('sounds/valor.mp3');
    const audioSoma = new Audio('sounds/soma.mp3');
    const audioSubtracao = new Audio('sounds/subtracao.mp3');

    // Verificar o contexto atual do jogo (nível 1 ou nível 2)
    if (jogadas < 2) {
        console.log('Tocando áudio pergunta');
        playSound(audioPergunta);
    } else if (jogadas >= 2 && jogadas < 5) {
        const operacao = (dificuldadeSelecionada === 'Facil') ? 'soma' : 'subtracao';
        if (operacao === 'soma') {
            console.log('Tocando áudio soma');
            playSound(audioSoma);
        } else {
            console.log('Tocando áudio subtracao');
            playSound(audioSubtracao);
        }
    }
});
});
